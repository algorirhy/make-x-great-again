...
X 原生拉黑；以及避免直接使用原项目的即时自动拉黑，在大量命中时集中发出请求。
推荐组合是把需要处理的自动类别设为**本地隐藏**，再开启**延迟自动 X 拉黑**：页面
立即变干净，X 拉黑则在已登录、账号匹配的 X 页面中按 45～75 秒一个、60/小时、
360/滚动 24 小时的限制逐个执行。

它不会拦截或减速仍配置为“X 拉黑”的即时动作。数字 userId 优先，缺失时才使用合法
handle；静音和安全封顶产生的本地隐藏不会升级。完整需求、动作分流、状态机、错误处理和
风险边界见 [设计与使用说明](./docs/DELAYED-AUTO-BLOCK.md)；旧商店版数据导入见
[数据迁移说明](./docs/DELAYED-AUTO-BLOCK-MIGRATION.md)。
...
> 个人 fork 用户如果希望降低自动拉黑的突发频率，应使用“自动本地隐藏 + 延迟自动 X 拉黑”，
> 不要同时为大量类别配置即时 X 拉黑。延迟限速只能降低突发请求，不能保证不触发 X 风控。
...
extension/            MV3 浏览器扩展：WXT + React 19 + Tailwind v4（名单下载、本地匹配）
  ...
  lib/                cache / blocklist / local-index / detect / stats / x-action / delayed-block
