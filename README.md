...
- **延迟自动 X 拉黑（个人 Chrome 构建，默认关闭）**：可将明确选择为本地隐藏的账号按 45～75 秒一个、60/小时、360/滚动 24 小时的限制逐个升级为 X 原生拉黑；优先使用数字 userId，缺失时以合法 handle 兜底。静音和被安全策略封顶的隐藏不会升级。任务仅在绑定账号的已登录 X 页面中运行，详细边界、handle 改名风险和商店版数据迁移见 [`docs/DELAYED-AUTO-BLOCK.md`](./docs/DELAYED-AUTO-BLOCK.md)
