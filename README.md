...
推荐把自动类别设为**本地隐藏**，再开启**延迟自动 X 拉黑**：内容立即隐藏，X 原生
拉黑在已登录且账号匹配的 X 页面中按 45～75 秒/次、60 次/小时、720 次/滚动 24 小时
逐个执行。静音、安全封顶动作和已恢复显示的记录不会升级；即时“X 拉黑”仍走原有队列。

行为、风险和状态说明见 [延迟自动 X 拉黑](./docs/DELAYED-AUTO-BLOCK.md)；从商店版
导入本地记录见 [数据迁移](./docs/DELAYED-AUTO-BLOCK-MIGRATION.md)。
