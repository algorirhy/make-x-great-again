...
新记录保存结构化的请求动作、实际动作和延迟拉黑资格，只有实际收到 X 成功响应
才写入成功账本。旧记录没有可靠的响应状态：明确的静音记录排除，其余具有数字
userId 或合法 handle 的历史记录全部加入，包括理由写着“自动拉黑”的旧记录。旧版
会在发送请求前写入这段文字，不能据此证明成功。这样可能对已经拉黑的账号重复提交
一次，但不会因不可靠的历史文字而漏掉账号；收到成功响应后不会继续重复。
...
- `404` 归类为“目标不存在/不可用”，不重试、不使用 handle 回退，也不计入失败；
- `401` / `403` 停止，必须关闭再重新开启；
- `429` 至少冷却一小时，并尊重更长的 `Retry-After`；
- 网络错误、`408` / `425` / `5xx` 最多尝试三次，退避 5 / 15 分钟；
- 其他客户端错误记为失败，不无限重试。

设置页顶部单独统计“目标不可用”，处理记录中显示相同状态；悬停仍可查看
`HTTP 404` 和尝试次数。升级后，已有的 `404` 失败记录会自动迁移到该状态，
不会再次进入队列。
