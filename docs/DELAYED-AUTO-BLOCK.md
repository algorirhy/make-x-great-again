...
明确的静音记录排除，明确的拉黑成功记录跳过，其余具有数字 userId 的历史记录
默认加入。这意味着少数历史状态不明账号可能被重复拉黑一次，或从旧静音升级为
拉黑；成功状态写入后不会继续重复。

第一版只自动处理数字 userId。`h:<handle>` 记录保留为本地隐藏，但不会自动拉黑，
避免用户名修改或被其他账号重新注册后误操作。
...
上述 30/小时和 200/24小时只约束延迟队列。用户明确配置的即时 X 静音/拉黑仍走
...
| `xss:delayed-block:states:v1` | 每个数字 userId 的队列/成功状态 |
