...
> 请求只发往 x.com，不经过我们的服务器、不收集任何数据（见下文 A.5）。
> `delayed-auto-block` 个人分支还可单独开启延迟拉黑；它使用相同数据流，并在本地保存
> 队列、执行状态和绑定的公开 X handle（见下文 A.5）。
...
- **设置项**：总开关、气泡开关与位置等
- **个人分支的延迟拉黑数据**：队列、请求时间戳、成功 / 失败状态及绑定的公开 X handle
...
`delayed-auto-block` 个人分支还提供一个默认关闭的独立开关。开启后，符合条件的本地
隐藏记录会在绑定账号与当前登录账号一致的 X 页面中限速拉黑；关闭只暂停。队列、账号
绑定和结果仅保存在 `chrome.storage.local`，不会上传。详见
[延迟自动 X 拉黑](./DELAYED-AUTO-BLOCK.md)。
